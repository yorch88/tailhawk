                ...
                <tbody>
                  {users.map(user => <tr key={user.id} className="text-default-800 font-normal text-sm whitespace-nowrap">
                      ...
                      <td className="flex py-3 px-3.5 items-center gap-3">
                        {user.avatar ? <Image src={user.avatar} alt={user.name} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 flex items-center justify-center rounded-full bg-default-200 font-semibold">
                            {user.initials}
                          </div>}
                        <div>
