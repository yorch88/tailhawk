...
const Chats = () => {
  return <div className="w-80">
      ...
      <div className="lg:h-[calc(100vh-310px)] h-[calc(100vh-125px)]" data-simplebar>
        ...
        {chats.filter(chat => chat.type === 'recent').map(chat => <Link key={chat.id} href="#" className={`flex gap-3 py-2 px-4 ${chat.id === 1 ? 'bg-default-200' : 'hover:bg-default-150'}`}>
              <div className="relative flex items-center justify-center font-semibold rounded-full size-9 bg-default-200">
                {chat.image ? <Image src={chat.image} alt={chat.name} width={36} height={36} className="rounded-full w-9 h-9" /> : chat.initials}
        ...
        {chats.filter(chat => chat.type === 'all').map(chat => <Link key={chat.id} href="#" className="flex gap-3 py-2 px-4 hover:bg-default-150">
              <div className="relative flex items-center justify-center font-semibold rounded-full size-9 bg-default-200">
                {chat.image ? <Image src={chat.image} alt={chat.name} width={36} height={36} className="rounded-full w-9 h-9" /> : chat.initials}
                <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 border-2 border-default-50 rounded-full ${statusColor[chat.status]}`}></span>
              </div>
