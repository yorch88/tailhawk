      ...
      <div className="card-header">
        ...
        <div className="hs-dropdown relative inline-flex">
          ...
          <div className="hs-dropdown-menu" role="menu">
            {['1 Weekly', '1 Monthly', '3 Monthly', '6 Monthly', 'This Yearly'].map((item, index) => <Link key={index} className="flex items-center gap-x-3.5 py-1.5 font-medium px-3 text-default-600 hover:bg-default-150 rounded" href="">
                  {item}
                </Link>)}
          </div>
        </div>
          ...
          {products.map(product => <div key={product.id} className="flex items-center justify-between">
              <div className="flex items-center gap-3 whitespace-nowrap">
                <div className="flex items-center justify-center rounded-md size-10 bg-default-100">
                  <Image src={product.image} alt={product.name} className="h-6" height={23} width={23} />
                </div>
                <div>
