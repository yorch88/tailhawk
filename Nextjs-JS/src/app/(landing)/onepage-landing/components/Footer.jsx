          ...
          {footerSections.map((section, idx) => <div key={idx} className="lg:col-span-3">
              ...
              <ul className="flex flex-col gap-y-3 text-sm">
                {section.links.map((link, i) => <li key={i}>
                    <Link href={link.href} className="relative inline-block transition-all duration-200 ease-linear text-default-400 before:absolute before:border-b before:border-default-400 before:inset-x-0 before:bottom-0 before:w-0 hover:before:w-full before:transition-all before:duration-300 before:ease-linear">
                      {link.label}
                    </Link>
                  </li>)}
              </ul>
        ...
        <div className="py-5 mt-20 border-y border-default-400/40">
          <div className="md:flex justify-between items-center">
            <div className="hs-dropdown relative inline-flex">
              ...
              <div className="hs-dropdown-menu p-2 transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-38 card z-40 mt-2 divide-y divide-default-200" role="menu" aria-orientation="vertical" aria-labelledby="hs-dropdown-with-icons">
                <div className="p-1 space-y-3 text-sm font-normal">
                  {languages.map((lang, i) => <Link key={i} href="#" className="flex items-center gap-x-3.5 px-3 text-default-600 hover:text-primary">
            ...
            <div className="flex flex-wrap gap-3 md:mt-0 mt-5">
              ...
              return <Link key={i} href={social.href} className="size-10 btn border border-default-400/40 bg-transparent rounded-full text-default-400 hover:text-primary flex items-center justify-center">
