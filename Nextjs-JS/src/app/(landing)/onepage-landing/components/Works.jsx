          ...
          {workCards.map((card, index) => <div key={card.id} className={`card rounded-md shadow-md transition-all duration-300 hover:-translate-y-2 ${index === 0 ? '-mt-36' : 'lg:-mt-36'}`}>
              <div className="card-body">
                <div className="flex flex-col gap-y-6">
                  <Image src={card.image} alt={card.title} width={400} height={250} className="rounded-md shadow" />
                  <div>
                    ...
                    <Link href={card.link} className="text-primary text-base flex items-center gap-1.5">
                      Read More
