<template>
  <div class="card">
    <div class="card-header">
      ...
      <button class="btn btn-sm bg-primary text-white"><Icon icon="lucide:plus" class="size-4 me-1"></Icon>Add Holiday</button>
    </div>
          ...
          <div class="overflow-hidden">
            <table class="min-w-full divide-y divide-default-200">
              ...
              <tbody class="divide-y divide-default-200">
                <tr class="text-default-800 font-normal whitespace-nowrap divide-x divide-default-200" v-for="(item, idx) in holidays" :key="idx">
                  <td class="px-3.5 py-3 text-sm">{{ item.id }}</td>
                  <td class="px-3.5 py-3 text-sm">{{ item.day }}</td>
                  <td class="px-3.5 py-3 text-sm">{{ item.date }}</td>
                  <td class="px-3.5 py-3 text-sm">{{ item.name }}</td>
                  <td class="px-3.5 py-3 text-sm">{{ item.type }}</td>
                  <td class="px-3.5 py-3">
                    <div class="flex items-center gap-2">
                      <RouterLink to="" class="flex size-8 bg-default-200 rounded-md items-center justify-center hover:bg-primary/10 hover:text-primary transition-all text-default-600" aria-haspopup="dialog" aria-expanded="false" aria-controls="holidaysLeaveDeleteModal2" data-hs-overlay="#holidaysLeaveDeleteModal2">
                        <Icon icon="lucide:pencil" class="size-4"></Icon>
                      </RouterLink>
                      <RouterLink to="" class="flex size-8 bg-default-200 rounded-md items-center justify-center hover:bg-primary/10 hover:text-primary transition-all text-default-600" aria-haspopup="dialog" aria-expanded="false" aria-controls="holidaysLeaveDeleteModal" data-hs-overlay="#holidaysLeaveDeleteModal">
