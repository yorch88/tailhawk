    ...
    <div class="card" v-for="(item, idx) in cards" :key="idx">
      <div class="card-body">
        <div class="flex flex-col justify-between h-full">
          <div class="mb-5 flex justify-between items-center gap-2">
            <div class="flex gap-2 items-center">
              <div class="hs-dropdown relative inline-flex">
                <button type="button" :class="item.buttonBg" class="hs-dropdown-toggle size-4 border border-dashed rounded-full dropdown-toggle shrink-0" aria-haspopup="menu" aria-expanded="false" aria-label="Dropdown"></button>
                <div class="hs-dropdown-menu" role="menu">
                  <div>
                    <a class="flex items-center gap-x-3.5 py-1.5 font-medium px-3 text-sm text-default-600 hover:bg-default-150 rounded" href=""> Personal </a>
                    <a class="flex items-center gap-x-3.5 py-1.5 font-medium px-3 text-sm text-default-600 hover:bg-default-150 rounded" href=""> Busssiness </a>
                    <a class="flex items-center gap-x-3.5 py-1.5 font-medium px-3 text-sm text-default-600 hover:bg-default-150 rounded" href=""> Social </a>
                    <a class="flex items-center gap-x-3.5 py-1.5 font-medium px-3 text-sm text-default-600 hover:bg-default-150 rounded" href=""> Home </a>
                  </div>
                </div>
              </div>
          ...
          <p class="text-default-700 text-sm" v-html="item.description"></p>
          <div v-if="item.date" class="mt-4 flex justify-between gap-3 items-center">
            <Icon icon="lucide:star" :class="item.class" class="size-5"></Icon>
            <p class="text-default-500 text-sm">{{ item.date }}</p>
          </div>
        </div>
      </div>
    </div>
